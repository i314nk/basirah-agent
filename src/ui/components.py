...
def render_header():
    """Render application header with branding (Phase 9: Framework-Driven)"""
    st.markdown(
        '<h1 style="color: #1f77b4; font-size: 2.5rem;">📈 basīrah</h1>',
        unsafe_allow_html=True
    )
    st.markdown("**Buffett Framework Investment Analysis**")
    st.markdown(
        "Systematic investment analysis using Warren Buffett's 8 core principles "
        "and Charlie Munger's mental models"
    )
    st.divider()
...
def render_progress_info(deep_dive: bool):
    ...
    if deep_dive:
        st.info(
            "📊 **Deep Dive Analysis (Phase 9.1: Tiered, GuruFocus-First)**\n\n"
            "**Tier 1: Quick Screen (All Companies) - Decision Gate**\n"
            "• GuruFocus: 10-year quantitative data (ROIC, margins, debt, Owner Earnings)\n"
            "• Latest 10-K (full): Business description, moat, risks, competitive position\n"
            "• Web search: Moat validation, recent news, competitive dynamics\n"
            "• **Decision Gate:** AVOID (40-50%) → STOP | WATCH (40-50%) → STOP | BUY candidate (10-20%) → Proceed to Tier 2\n\n"
            "**Tier 2: Deep Dive (BUY Candidates Only - 10-20% of companies)**\n"
            "• 5 years of MD&A sections (NOT full 10-Ks): Management track record over time\n"
            "• Proxy statement (DEF 14A): CEO compensation, insider ownership, related parties\n"
            "• Targeted web search: Key acquisitions, strategic decisions, competitive responses\n"
            "• Charlie Munger critique: Mental models validation (Inversion, Second-Order Thinking)\n"
            "• Final framework scoring: Moat (/15), Management (/12), Predictability (/3)\n\n"
            "**Cost Efficiency:**\n"
            "• Tier 1 only (80-90% of companies): ~$1-2 (AVOID/WATCH → early exit)\n"
            "• Tier 1 + Tier 2 (10-20% of companies): ~$3-5 (BUY candidates only)\n\n"
            "**Expected Time:**\n"
            "• Tier 1: 2-4 minutes\n"
            "• Tier 2 (if BUY candidate): +3-5 minutes"
        )
    else:
        st.info(
            "⚡ **Quick Screen**\n\n"
            "The analysis framework will:\n"
            ...
            with st.expander("📊 Context Management Details"):
                ...
                if cm.get('adaptive_used', False):
                    st.info(
                        ...
                        f"The agent read the complete filing and created a comprehensive summary "
...
*Generated by basīrah - Framework-Driven Investment Analysis Agent*
"""
    return md
...
def render_sidebar_info():
    ...
    with st.sidebar:
        ...
        st.markdown("### 📊 Framework-Driven Analysis (Phase 9)")

        st.markdown("""
        **Buffett's 8 Core Principles:**

        1. **Circle of Competence** - Simple, understandable businesses only
        2. **Economic Moat** - Durable competitive advantages (scored /15)
        3. **Management Quality** - Capital allocation excellence (scored /12)
        4. **Margin of Safety** - ≥25% discount for BUY
        5. **Predictability** - 10+ year business visibility (scored /3)
        6. **Owner Earnings** - True cash flow, not GAAP
        7. **Quality Over Quantity** - BUY is rare (5-10% of companies)
        8. **Long-Term Focus** - Forever holding period

        **Validation:** Charlie Munger's Mental Models
        - Inversion, Second-Order Thinking, Incentive Analysis
